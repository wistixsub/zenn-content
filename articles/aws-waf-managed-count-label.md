...
`Block` で投入すると、これらが**本番でいきなり起きます**。WAF導入直後に問い合わせが止まって慌てる、という典型パターンです。
...
- そして `labels` 配列に `awswaf:managed:aws:core-rule-set:CrossSiteScripting_Body` が付きます。**後続ルールや Logs Insights が拾うのはこのラベル**です。他のルールも同様に、`...:NoUserAgent_Header` や `awswaf:managed:aws:sql-database:SQLi_QueryArguments` のような文字列で `labels` に残ります。
...
> ⚠️ **地味な罠：ルール名とラベル名でケース（大文字小文字）が違います**
...
:::message
本記事冒頭の私のケースは、実はこのパターンBの途中です。すでに適用されていたルールが誤検知 → 優先度の都合でいったん Count に落とし → これから安全に Block へ戻していく、という流れでした。
:::
...
- 「いきなり Block で事故る」も「Count のままで守れない」も避けられる、現場で回る落とし所だと思います。
